...
shogoa service development begins with writing the *design* of a service. The design is described using
the shogoa language implemented by the github.com/shogo82148/shogoa/design/apidsl package. The `shogoagen` tool
...
The shogoa package contains supporting functionality for the generated code including basic request
...
The RequestData and ResponseData structs provides access to the request and response state. shogoa request
...
where UpdateBottleContext is:
	...
	type UpdateBottleContext struct {
	    ...
	    *shogoa.ResponseData    // Response state access
	    *shogoa.RequestData     // Request state access
	    Service *shogoa.Service // Service handling request
...
an anonymous field of type *shogoa.Controller which takes care of implementing the middleware handling.

# Middleware

A shogoa middleware is a function that takes and returns a Handler. A Handler is a the low level
function which handles incoming HTTP requests. shogoagen generates the handlers code so each handler
creates the action specific context and calls the controller action with it.

Middleware can be added to a shogoa service or a specific controller using the corresponding Use
methods. shogoa comes with a few stock middleware that handle common needs such as logging, panic
recovery or using the RequestID header to trace requests across multiple services.

# Error Handling

The controller action methods generated by shogoagen such as the Update method of the BottleController
interface shown above all return an error value. shogoa defines an Error struct that action
...
The shogoa design language documented in the dsl package makes it possible to attach validations to
...
code generated by shogoagen.

# Encoding

The shogoa design language makes it possible to specify the encodings supported by the API both as
input (Consumes) and output (Produces). shogoagen uses that information to registered the corresponding
