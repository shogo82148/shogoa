...
//
// While keys can have any value the following names are handled explicitly by shogoagen when set on
// attributes.
//
// `struct:field:name`: overrides the Go struct field name generated by default by shogoagen.
// Applicable to attributes only.
//
//	Metadata("struct:field:name", "MyName")
//
// `struct:field:type`: overrides the Go struct field type generated by default by shogoagen.
...
// shogoagen would otherwise set.  If the metadata value is a slice then the strings are joined with
