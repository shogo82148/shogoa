...
func CheckVersion(ver string) error {
	...
	if !compat {
		return fmt.Errorf("version mismatch: using shogoagen %s to generate code that compiles with shogoa %s",
			ver, version.String())
	}
	return nil
}
...
func CommandLine() string {
	// We don't use the full path to the tool so that running shogoagen multiple times doesn't
